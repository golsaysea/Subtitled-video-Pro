...
import {
  ...
  type ElevenLabsBridge,
  type ElevenLabsState,
  type ElevenLabsVoice
} from "../shared/qwebchannel";
...
type ImportResult = {
  ...
  state?: Partial<ElevenLabsState>;
  message?: string;
};

const defaultState: ElevenLabsState = {
  accounts: [],
  currentKey: "",
  voiceId: "",
...
let bridge: ElevenLabsBridge | null = null;
let saveTimer = 0;
const state: ElevenLabsState & { currentQuota?: number | null; currentLimit?: number | null } = {
  ...defaultState,
  currentQuota: null,
  currentLimit: null
};
...
function applySpreadsheetPaste(event: ClipboardEvent, index: number): void {
...
function currentAccount(): ElevenLabsAccount | null {
  return state.accounts.find((item) => item.key === state.currentKey) ?? null;
}

function quotaText(account: ElevenLabsAccount | null): string {
...
function collectControls(): void {
  state.outputDir = byId<HTMLInputElement>("output-dir").value.trim();
  state.voiceId = byId<HTMLInputElement>("voice-id").value.trim() || byId<HTMLSelectElement>("voice-select").value || state.voiceId;
  state.model = byId<HTMLSelectElement>("model").value;
  state.format = byId<HTMLSelectElement>("format").value;
  state.stability = Number(byId<HTMLInputElement>("stability").value);
  state.similarity = Number(byId<HTMLInputElement>("similarity").value);
  state.style = Number(byId<HTMLInputElement>("style").value);
  state.speakerBoost = byId<HTMLInputElement>("speaker-boost").checked;
  state.clearAfter = byId<HTMLInputElement>("clear-after").checked;
  state.apiKeyLink = byId<HTMLInputElement>("api-key-link").value.trim() || defaultState.apiKeyLink;
}
...
function renderControls(): void {
  byId<HTMLInputElement>("output-dir").value = state.outputDir || "";
  byId<HTMLInputElement>("voice-id").value = state.voiceId || "";
  byId<HTMLSelectElement>("model").value = state.model || defaultState.model;
  byId<HTMLSelectElement>("format").value = state.format || defaultState.format;
  byId<HTMLInputElement>("stability").value = String(state.stability ?? 0.5);
  byId<HTMLInputElement>("similarity").value = String(state.similarity ?? 0.75);
  byId<HTMLInputElement>("style").value = String(state.style ?? 0);
  ...
  byId<HTMLInputElement>("speaker-boost").checked = state.speakerBoost !== false;
  byId<HTMLInputElement>("clear-after").checked = Boolean(state.clearAfter);
  byId<HTMLInputElement>("api-key-link").value = state.apiKeyLink || defaultState.apiKeyLink;
}
...
function renderAccount(): void {
  ...
  const select = byId<HTMLSelectElement>("account-select");
  select.innerHTML = "";
...
function renderVoices(): void {
  const select = byId<HTMLSelectElement>("voice-select");
  select.innerHTML = "";
  ...
  if (state.voiceId && [...select.options].some((option) => option.value === state.voiceId)) {
    ...
    byId<HTMLInputElement>("voice-id").value = "";
  }
}
...
function renderCards(): void {
  ...
  state.cards.forEach((text, index) => {
    ...
    const count = card.querySelector("b") as HTMLElement;
...
function addOrSwitchAccount(alias: string, key: string): void {
...
function switchAccount(key: string, shouldPersist = true): void {
...
function setProgress(value: number): void {
  byId<HTMLSpanElement>("progress").style.width = `${Math.max(0, Math.min(100, value))}%`;
}
...
function onBridgeEvent(raw: string): void {
  const event = parseJson<BridgeEvent>(raw, {});
  ...
  if (event.type === "generationFinished") byId<HTMLButtonElement>("generate").disabled = false;
}

function normalizeImportedState(source: unknown): Partial<ElevenLabsState> {
  const data = source as Record<string, unknown>;
  if (!data || typeof data !== "object") return {};
  if (typeof data.elevenlabs_tool === "object") return normalizeImportedState(data.elevenlabs_tool);
  return data as Partial<ElevenLabsState>;
}
...
function wireUi(): void {
  ...
  byId<HTMLSelectElement>("account-select").addEventListener("change", () => {
    const key = byId<HTMLSelectElement>("account-select").value;
    if (key) switchAccount(key);
  });
  byId("quick-add").addEventListener("click", () => {
    addOrSwitchAccount("", byId<HTMLInputElement>("quick-key").value);
    byId<HTMLInputElement>("quick-key").value = "";
  });
  byId<HTMLInputElement>("quick-key").addEventListener("keydown", (event) => {
  ...
  byId("pick-output").addEventListener("click", () => bridge?.selectOutputDir((path) => {
    ...
    byId<HTMLInputElement>("output-dir").value = path;
    persist();
  }));
  ...
  byId("new-key-save").addEventListener("click", () => {
    addOrSwitchAccount(byId<HTMLInputElement>("new-alias").value, byId<HTMLInputElement>("new-key").value);
    byId<HTMLInputElement>("new-alias").value = "";
    byId<HTMLInputElement>("new-key").value = "";
  });
  ...
  byId("import-keys").addEventListener("click", () => bridge?.importAccountsCsv((raw) => {
    const result = parseJson<ImportResult>(raw, {});
    let added = 0;
  ...
  byId("export-keys").addEventListener("click", () => bridge?.exportAccountsCsv(JSON.stringify(state.accounts), (raw) => {
    setStatus(parseJson<ImportResult>(raw, {}).message || "");
  }));
  ...
  ["output-dir", "voice-id", "model", "format", "stability", "similarity", "style", "speaker-boost", "clear-after", "api-key-link"]
    .forEach((id) => byId(id).addEventListener("input", () => {
      if (id === "stability" || id === "similarity" || id === "style") {
        byId(`${id}-value`).textContent = Number(byId<HTMLInputElement>(id).value).toFixed(2);
      }
      persistSoon();
    }));
  byId("voice-select").addEventListener("change", () => {
    state.voiceId = byId<HTMLSelectElement>("voice-select").value;
    byId<HTMLInputElement>("voice-id").value = "";
    persist();
  });
}
...
connectQtBridge<ElevenLabsBridge>("elevenlabsBridge")
  .then((connectedBridge) => {
    bridge = connectedBridge;
    byId("ready-pill").textContent = "已连接";
    bridge.event.connect(onBridgeEvent);
    bridge.getState((raw) => {
      Object.assign(state, defaultState, parseJson<Partial<ElevenLabsState>>(raw, {}));
      if (!state.cards.length) state.cards = [""];
      renderAll();
      if (state.currentKey) {
