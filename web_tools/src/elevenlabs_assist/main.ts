...
import {
  connectQtBridge,
  parseJson,
  type ElevenLabsAssistAccount,
  type ElevenLabsAssistBridge,
  type ElevenLabsAssistState,
...
const defaultState: ElevenLabsAssistState = {
  accounts: [],
  currentToken: "",
  voiceId: "",
...
let bridge: ElevenLabsAssistBridge | null = null;
let saveTimer = 0;
const state: ElevenLabsAssistState = { ...defaultState };
...
function currentAccount(): ElevenLabsAssistAccount | null {
  return state.accounts.find((item) => item.token === state.currentToken) ?? null;
}

function quotaText(account: ElevenLabsAssistAccount | null): string {
...
function collectControls(): void {
  ...
  state.subFolder = byId<HTMLInputElement>("sub-folder").value.trim() || "批量导出";
  ...
  state.compatMode = byId<HTMLInputElement>("compat-mode").checked;
  state.autoDelete = byId<HTMLInputElement>("auto-delete").checked;
}
...
function renderControls(): void {
  ...
  byId<HTMLInputElement>("sub-folder").value = state.subFolder || "批量导出";
  ...
  byId<HTMLInputElement>("compat-mode").checked = state.compatMode !== false;
  byId<HTMLInputElement>("auto-delete").checked = state.autoDelete !== false;
}
...
function switchAccount(token: string, shouldPersist = true): void {
...
function onBridgeEvent(raw: string): void {
  const event = parseJson<AssistEvent>(raw, {});
...
function wireUi(): void {
  ...
  byId<HTMLSelectElement>("account-select").addEventListener("change", () => {
    const token = byId<HTMLSelectElement>("account-select").value;
  ...
  ["output-dir", "sub-folder", "voice-id", "model", "stability", "similarity", "style", "speaker-boost", "compat-mode", "auto-delete"]
...
connectQtBridge<ElevenLabsAssistBridge>("elevenAssistBridge")
  .then((connectedBridge) => {
    bridge = connectedBridge;
    byId("ready-pill").textContent = "已连接";
    bridge.event.connect(onBridgeEvent);
    bridge.getState((raw) => {
      Object.assign(state, defaultState, parseJson<Partial<ElevenLabsAssistState>>(raw, {}));
      if (!state.cards.length) state.cards = [""];
      renderAll();
