...
class ClipItem(QGraphicsRectItem):
    ...
    def paint(self, painter, option, widget=None):
        ...
        if self.isSelected(): painter.setBrush(QBrush(QColor(c["danger"]))); painter.setPen(QPen(QColor(c["selected_text"]), 2))
        else: painter.setBrush(QBrush(QColor(clip_color))); painter.setPen(QPen(QColor(c["border"]), 1))
...
class AdvancedTimeline(QGraphicsView):
    ...
    def drawBackground(self, painter, rect):
        ...
        painter.setPen(QColor(c["border"])); painter.drawLine(QPointF(0, HEADER_H+TRACK_H*4), QPointF(w, HEADER_H+TRACK_H*4))
